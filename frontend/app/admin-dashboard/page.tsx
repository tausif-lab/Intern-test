...
import { Task, User, TasksResponse, UsersResponse, TaskResponse } from '@/types/auth';
...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [tasksRes, usersRes] = await Promise.all([
        ...
        apiFetch<UsersResponse>('/api/tasks/users'),
      ]);
  ...
  useEffect(() => {
    ...
    if (u.role !== 'admin') { router.push('/user-dashboard'); return; }
    setAdminUser(u);
    fetchData();
  }, [fetchData, router]);
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    if (!form.title || !form.assignedTo) { setFormError('Title and assignee are required.'); return; }
    setSubmitting(true);
    try {
      const res = await apiFetch<TaskResponse>('/api/tasks', {
        method: 'POST',
        body: JSON.stringify({ title: form.title, description: form.description, assignedTo: form.assignedTo, priority: form.priority, dueDate: form.dueDate || undefined }),
      });
  ...
  const getAssigneeName = (t: Task) => typeof t.assignedTo === 'object' ? t.assignedTo.fullName : t.assignedTo;
  ...
  const inputCls = 'w-full bg-slate-800/80 border border-indigo-500/20 rounded-xl py-2.5 px-3.5 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all';
    ...
    <div className="min-h-screen bg-[#070b14] text-slate-200 relative overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between px-6 py-4 border-b border-indigo-500/15 bg-[#070b14]/85 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-bold uppercase tracking-widest px-2 py-0.5 rounded-md bg-indigo-500/20 border border-indigo-500/40 text-indigo-300">Admin</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm text-slate-400 hidden sm:block">👋 {adminUser?.fullName}</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {stats.map(s => (
            <div key={s.label} className="bg-slate-900/70 border border-indigo-500/15 rounded-2xl p-5 flex items-center gap-4 backdrop-blur-sm hover:border-indigo-500/35 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 mb-6">
          ...
          <section className="bg-slate-900/70 border border-indigo-500/15 rounded-2xl p-6 backdrop-blur-sm">
            ...
            {formError && <div className="bg-red-500/10 border border-red-500/30 text-red-300 rounded-xl px-4 py-2.5 text-sm mb-4">{formError}</div>}
            {formSuccess && <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 rounded-xl px-4 py-2.5 text-sm mb-4">{formSuccess}</div>}

            <form onSubmit={handleCreateTask} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Task Title *</label>
                <input id="task-title" className={inputCls} placeholder="e.g. Design login page" value={form.title}
                  onChange={e => setForm({ ...form, title: e.target.value })} required />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Description</label>
                <textarea id="task-desc" className={`${inputCls} resize-none`} placeholder="Task details…"
                  value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={3} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Assign To *</label>
                  <select id="task-assignee" className={inputCls} value={form.assignedTo}
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select id="task-priority" className={inputCls} value={form.priority}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input id="task-due" className={inputCls} type="date" value={form.dueDate}
                  onChange={e => setForm({ ...form, dueDate: e.target.value })} />
              </div>
              <button id="create-task-btn" type="submit" disabled={submitting}
          ...
          <section className="bg-slate-900/70 border border-indigo-500/15 rounded-2xl p-6 backdrop-blur-sm">
            ...
            {users.length === 0 ? (
              ...
              <ul className="flex flex-col gap-2">
                {users.map(u => {
                  const taskCount = tasks.filter(t => (typeof t.assignedTo === 'object' ? t.assignedTo._id : t.assignedTo) === u._id).length;
                  return (
                    <li key={u._id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/50 border border-indigo-500/10 hover:border-indigo-500/25 transition-all">
                      ...
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-slate-200 truncate">{u.fullName}</div>
                        <div className="text-xs text-slate-500 truncate">{u.email}</div>
                      </div>
                      <span className="ml-auto text-xs text-indigo-300 bg-indigo-500/15 rounded-md px-2 py-0.5 whitespace-nowrap">{taskCount} tasks</span>
                    </li>
                  );
                })}
              </ul>
        ...
        <section className="bg-slate-900/70 border border-indigo-500/15 rounded-2xl p-6 backdrop-blur-sm">
          ...
          {loading ? (
            ...
            <p className="text-slate-500 text-sm text-center py-8">No tasks yet. Assign one above!</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-indigo-500/12">
                    {['Title', 'Assigned To', 'Priority', 'Status', 'Due Date', 'Action'].map(h => (
                      <th key={h} className="text-left text-xs font-semibold text-slate-500 uppercase tracking-widest pb-3 pr-4">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {tasks.map(t => (
                    <tr key={t._id} className="border-b border-indigo-500/07 hover:bg-indigo-500/04 transition-colors">
                      ...
                      <td className="py-3.5 pr-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-slate-300">{getAssigneeName(t)}</span>
                        </div>
                      </td>
                      <td className="py-3.5 pr-4">
                        <span className={`text-xs font-semibold border rounded-md px-2.5 py-1 capitalize ${priorityStyles[t.priority]}`}>
                          {t.priority}
                        </span>
                      </td>
                      <td className="py-3.5 pr-4">
                        <span className={`text-xs font-semibold border rounded-md px-2.5 py-1 capitalize ${statusStyles[t.status]}`}>
                          {t.status}
                        </span>
                      </td>
