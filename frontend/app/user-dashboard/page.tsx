...
export default function UserDashboard() {
  ...
  const fetchTasks = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiFetch<TasksResponse>("/api/v1/tasks");
  ...
  function TaskCard({ task }: { task: Task }) {
    ...
    return (
      <div
        className={`flex flex-col gap-3 bg-slate-900/75 border rounded-2xl p-5 backdrop-blur-sm transition-all duration-200 hover:-translate-y-0.5 ${isDone ? "border-indigo-500/10 opacity-60 hover:opacity-80" : "border-indigo-500/18 hover:border-indigo-500/35"}`}
      >
        {/* Top row */}
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <span
            className={`text-xs font-semibold border rounded-md px-2.5 py-1 ${pc.badge}`}
          >
            {pc.label}
          </span>
          {task.dueDate && (
            <span
              className={`text-xs flex items-center gap-1 ${isOverdue ? "text-red-400" : "text-slate-500"}`}
            >
        ...
        {/* Title */}
        <h3
          className={`text-base font-semibold leading-snug ${isDone ? "line-through text-slate-500" : "text-slate-100"}`}
        >
          {task.title}
        </h3>
        ...
        <div className="flex items-center justify-between text-xs text-slate-500">
          <span>
            Assigned by{" "}
            <span className="text-slate-400 font-medium">{assignedBy}</span>
          </span>
        ...
        <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-800">
          <span
            className={`text-xs font-semibold border rounded-md px-2.5 py-1 ${isDone ? "text-emerald-400 bg-emerald-500/10 border-emerald-500/30" : "text-amber-400 bg-amber-500/10 border-amber-500/30"}`}
          >
    ...
    <div className="min-h-screen bg-[#070b14] text-slate-200 relative overflow-x-hidden">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        {loading ? (
          ...
          <div className="bg-red-500/10 border border-red-500/30 text-red-300 rounded-2xl px-5 py-4 text-sm">
            {error}
          </div>
        ) : tasks.length === 0 ? (
          <div className="text-center py-24">
            ...
            <p className="text-slate-500 text-sm">
              Your admin hasn&apos;t assigned any tasks to you yet. Check back
              later!
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-8">
            {pending.length > 0 && (
              <section>
                <h2 className="flex items-center gap-2.5 text-base font-semibold text-slate-300 mb-4">
                  ⏳ Pending
                  <span className="text-xs font-bold bg-amber-500/15 border border-amber-500/30 text-amber-300 rounded-full px-2.5 py-0.5">
                    {pending.length}
                  </span>
                </h2>
            ...
            {completed.length > 0 && (
              <section>
                <h2 className="flex items-center gap-2.5 text-base font-semibold text-slate-300 mb-4">
                  ✅ Completed
                  <span className="text-xs font-bold bg-emerald-500/12 border border-emerald-500/30 text-emerald-300 rounded-full px-2.5 py-0.5">
                    {completed.length}
                  </span>
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
